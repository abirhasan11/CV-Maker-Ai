...
import { Mail, Phone, MapPin, Linkedin, Sun, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const CaliforniaSunnyTemplate: React.FC<TemplateProps> = ({ resume, config }) => {
  ...
  return (
    <div className={`w-full bg-[#faf8f5] text-slate-800 ${fontClass} min-h-[1050px] p-7 flex flex-col justify-between space-y-4`}>
      {/* Warm Sunny Top Header */}
      <div 
        className="rounded-3xl p-6 text-white shadow-sm flex justify-between items-center relative overflow-hidden"
        style={{ background: theme.gradient || theme.primary, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}
      >
        <div className="space-y-2 max-w-[75%]">
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-[11px] font-bold tracking-wide">
            <Sun className="w-3.5 h-3.5" />
            <span>{personalInfo.jobTitle || 'SQA & Automation Engineer'}</span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight">
            {personalInfo.fullName || 'Abir Hasan'}
          </h1>
          <div className="flex flex-wrap gap-2 text-xs pt-1">
            {personalInfo.email && <span className="bg-black/15 px-2.5 py-0.5 rounded-lg">{personalInfo.email}</span>}
            {personalInfo.phone && <span className="bg-black/15 px-2.5 py-0.5 rounded-lg">{personalInfo.phone}</span>}
            {personalInfo.location && <span className="bg-black/15 px-2.5 py-0.5 rounded-lg">{personalInfo.location}</span>}
          </div>
        </div>
      ...
      {personalInfo.summary && (
        <div className="bg-white p-4 rounded-2xl border border-amber-100 shadow-2xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700">Profile Overview</span>
      ...
      {experiences && experiences.length > 0 && (
        <div className="bg-white p-4 rounded-2xl border border-amber-100 shadow-2xs space-y-3">
          <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700">Experience</span>
            ...
            {experiences.map((exp) => (
              <div key={exp.id} className="page-break-avoid border-l-2 pl-3 space-y-1" style={{ borderColor: theme.primary }}>
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-[11px] font-medium text-amber-800 bg-amber-50 px-2 py-0.5 rounded-full">{exp.startDate} – {exp.isCurrent ? 'Present' : exp.endDate}</span>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
        {skills && skills.length > 0 && (
          <div className="bg-white p-4 rounded-2xl border border-amber-100 shadow-2xs space-y-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700">Skills & Mastery</span>
        ...
        {education && education.length > 0 && (
          <div className="bg-white p-4 rounded-2xl border border-amber-100 shadow-2xs space-y-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700">Education</span>
            <div className="space-y-2">
              {education.map((edu) => (
